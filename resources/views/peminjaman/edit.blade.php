            ...
            <div class="lg:col-span-2">
                ...
                <div class="mb-6">
                    <div class="bg-white p-4.5 shadow-md rounded-xl w-full">
                        ...
                        <p class="text-gray-500 text-sm mt-1">Ubah detail pengajuan peminjaman yang masih menunggu
                            persetujuan</p>
                    </div>
                </div>
                ...
                <div class="mb-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
                    <div class="flex items-start gap-3">
                        ...
                        <p class="text-sm text-blue-800">
                            Anda hanya dapat mengedit pengajuan yang masih dalam status <strong>Menunggu
                                Persetujuan</strong>
                        </p>
                    </div>
                </div>
                ...
                <form action="{{ route('peminjaman.update', $peminjaman->id) }}" method="POST">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="date" name="tanggal_pinjam"
                                        class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all duration-200 hover:border-gray-400 @error('tanggal_pinjam') border-red-500 @enderror"
                                ...
                                <div>
                                    ...
                                    <input type="date" name="tanggal_kembali_rencana"
                                        class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all duration-200 hover:border-gray-400 @error('tanggal_kembali_rencana') border-red-500 @enderror"
                    ...
                    <div class="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden mb-6">
                        <div class="px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-white">
                            <h2 class="text-lg font-semibold text-gray-800 flex items-center gap-2">
                                <i class='bx bx-package text-amber-600'></i>
                                Daftar Alat
                            </h2>
                        ...
                        <div class="p-6 border-b border-gray-100">
                            <div class="relative">
                                ...
                                <input type="text" id="toolSearch" placeholder="Cari alat berdasarkan nama atau kode..."
                                    class="w-full pl-12 pr-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all">
                            </div>
                        </div>
                            ...
                            <table class="w-full" id="toolsTable">
                                ...
                                <tbody class="divide-y divide-gray-100">
                                    @forelse($alat as $item)
                                        @php
                                            $detail = $peminjaman->detailPeminjaman->firstWhere('alat_id', $item->id);
                                        @endphp
                                        <tr class="hover:bg-gray-50 transition-colors tool-row"
                                            ...
                                            <td class="px-6 py-4">
                                                <input type="checkbox" name="alat_id[]" value="{{ $item->id }}"
                                                    class="itemCheckbox h-4 w-4 cursor-pointer rounded border-gray-300 text-amber-600 focus:ring-amber-500"
                                                    {{ $detail ? 'checked' : '' }} onchange="toggleQuantityInput(this)">
                                            </td>
                                            <td class="px-6 py-4">
                                                <span
                                            ...
                                            <td class="px-6 py-4">
                                                <div class="flex items-center gap-3">
                                                    <div
                                                        class="w-8 h-8 bg-gradient-to-br from-amber-400 to-orange-500 rounded-lg flex items-center justify-center text-white">
                                            ...
                                            <td class="px-6 py-4 text-center">
                                                <input type="number"
                                                    class="quantityInput w-24 border border-gray-300 rounded-lg px-3 py-2 text-center focus:ring-2 focus:ring-amber-500 focus:border-transparent"
                                                    name="jumlah[{{ $item->id }}]" min="1"
                                                    max="{{ $item->stok }}" value="{{ $detail->jumlah ?? '' }}"
                                                    placeholder="0" {{ $detail ? '' : 'disabled' }}
            ...
            <div class="lg:col-span-1 space-y-6">
                    ...
                    <div class="space-y-3">
                        <div class="p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                            <div class="flex items-center gap-2 mb-1">
                                <i class='bx bx-time-five text-yellow-600 text-lg'></i>
                                <h5 class="font-semibold text-yellow-900 text-sm">Status Saat Ini</h5>
                            </div>
                            <p class="text-xs text-yellow-700">{{ ucfirst($peminjaman->status) }}</p>
                        </div>
                        ...
                        <div class="flex items-start gap-3 p-2 bg-gray-50 rounded-lg">
                            ...
                            <div>
                                ...
                                <p class="text-sm font-semibold text-gray-800">
                                    {{ $peminjaman->created_at->format('d M Y, H:i') }}</p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-start gap-3 p-2 bg-gray-50 rounded-lg">
                            ...
                            <div>
                                ...
                                <p class="text-sm font-semibold text-gray-800">
                                    {{ $peminjaman->updated_at->format('d M Y, H:i') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
                    <h4 class="text-sm font-bold text-gray-800 mb-4 flex items-center gap-2">
                        ...
                        Alat Dipinjam Saat Ini
                    </h4>
                    <div class="space-y-2">
                        @foreach ($peminjaman->detailPeminjaman as $detail)
                            <div class="p-2 bg-gray-50 rounded-lg">
                                <div class="flex items-center justify-between">
                                    <span class="text-sm font-medium text-gray-700">{{ $detail->alat->nama_alat }}</span>
                                    <span
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-blue-800 leading-relaxed">
                                Jika mengubah alat atau jumlah, pastikan stok masih tersedia untuk menghindari penolakan.
                            </p>
                        </div>
                    </div>
                </div>
